...
from servers.xmlrpc.insult_service import run_server as xmlrpc_run_server
from servers.redis.insult_service import run_server as redis_run_server
from servers.pyro.insult_service import run_server as pyro_run_server
...
class SingleNodeStressTesterInsultService:
    ...
    def run_stress_test(self, client_class, max_clients=50, requests_per_client=100):
        ...
        for client_count in client_counts:
            ...
            def client_work(idx):
                try:
                    client = client_class(port=10000+ idx) if client_class.__name__ == 'InsultServiceXMLRPCClient' else client_class()
            ...
            t = []
            for cli in range(client_count):
                a = threading.Thread(target=client_work, args=([cli]))
                t.append(a)
                a.start()
            for a in t:
                a.join()
...
if __name__ == "__main__":
    ...
    tester = SingleNodeStressTesterInsultService(Process(target=xmlrpc_run_server))
    tester.run_stress_test(InsultServiceXMLRPCClient, max_clients=40, requests_per_client=50)

    print("Testing Redis insult filter (single node) ...")
    tester = SingleNodeStressTesterInsultService(Process(target=redis_run_server))
    tester.run_stress_test(InsultServiceRedisClient, max_clients=50, requests_per_client=50)

    print("Testing Pyro insult filter (single node) ...")
    tester = SingleNodeStressTesterInsultService(Process(target=pyro_run_server))
    tester.run_stress_test(InsultServicePyroClient, max_clients=50, requests_per_client=50)
